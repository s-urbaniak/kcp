...
func TestMaximalPermissionPolicyAuthorizerSystemGroupProtection(t *testing.T) {
	...
	framework.AdmitNewWorkspaceAccess(t, ctx, kubeClusterClient, orgClusterName, []string{"user-1"}, nil, []string{"admin", "access"})
...
func TestMaximalPermissionPolicyAuthorizer(t *testing.T) {
	...
	for serviceProvider, consumer := range m {
		...
		framework.AdmitNewWorkspaceAccess(t, ctx, kubeClusterClient, consumer, []string{"user-1", "user-2", "user-3"}, nil, []string{"access"})
